...
$(document).ready(function() {
	...
	$('.export-link').on('click', function(e) {
		...
		$content.html('<small>URL: <a href="'+url+'">'+url+'</a></small><textarea class="form-control">Loading...</textarea>');
	...
	<div class="row">
		<aside class="col-xs-2" style="width:21%;">
			<ul class="nav nav-pills nav-stacked">
				<li class="active" data-id="import"><a
						href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=utils&pill=import#tabs-utils">Import</a>
				</li>
				<li data-id="export"><a
						href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=utils&pill=export#tabs-utils">Export</a>
				</li>
				<li data-id="api-explorer"><a
						href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=utils&pill=api-explorer#tabs-utils">API
						Explorer</a></li>
				<? if ($login_is_super): ?>
					<li class="admin" data-id="google-authenticator"><a
							href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=utils&pill=google-authenticator#tabs-utils">Google
							Authenticator</a></li>
					<li class="admin" data-id="disallowed-emails"><a
							href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=utils&pill=disallowed-emails#tabs-utils">Disallowed
							emails</a></li>
					<li class="admin" data-id="manage-users"><a
							href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=all-users&pill=manage-users#tabs-utils">Manage
							users</a></li>
					<li class="admin" data-id="manage-books"><a
							href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=all-books&pill=manage-books#tabs-utils">Manage
							books</a></li>
					<li class="admin" data-id="generate-email-list"><a
							href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=utils&pill=generate-email-list#tabs-utils">Generate
							email list</a></li>
					<li class="admin" data-id="recreate-book-folders"><a
							href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=utils&pill=recreate-book-folders#tabs-utils">Recreate
							book folders</a></li>
					<li class="admin" data-id="list-all-users"><a
							href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=utils&pill=list-all-users#tabs-utils">List
							all users</a></li>
					<li class="admin" data-id="list-all-books"><a
							href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=utils&pill=list-all-books#tabs-utils">List
							all books</a></li>
					<li class="admin" data-id="list-recent-pages"><a
							href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=utils&pill=list-recent-pages#tabs-utils">List
							recently edited pages</a></li>
					<li class="admin" data-id="normalize-id2val"><a
							href="?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : '0') ?>&zone=utils&pill=normalize-id2val#tabs-utils">Normalize
							predicate table</a></li>
				<? endif ?>
			</ul>
		</aside>
		<section class="col-xs-10" style="width:79%;">
			<div class="section" id="import">
				<?php if ('import' == $pill): ?>
					<h4>Import</h4>
					<p class="m">Import content and relationships from public Scalar books, raw Scalar RDF-JSON, or comma
						seperated values files. For more information visit the <a
							href="http://scalar.usc.edu/works/guide2">Scalar 2 Guide</a>'s <a
							href="http://scalar.usc.edu/works/guide2/advanced-topics">Advanced Topics</a> path.</p><?php
	    ...
	    <?php endif; ?>
				...
				<?php if ('manage-users' == $pill): ?>
					<h4>
						<? if (!empty($register_key)): ?>
							<a href="javascript:void(null);" id="register_key" style="float:right;"
								class="btn btn-warning btn-sm" data-key="<?= implode(' OR ', $register_key) ?>">Show registration
								key<?= ((count($register_key) > 1) ? 's' : '') ?></a>
						<? else: ?>
					...
					<? if (isset($_GET['error']) && $_GET['error'] == 1): ?>
						<div class="alert alert-danger">You left out a required field<a style="float:right;"
								href="?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&pill=manage-users&zone=all-users#tabs-utils">clear</a>
						</div>
					<? elseif (isset($_GET['error']) && $_GET['error'] == 2): ?>
						<div class="alert alert-danger">Password and Retype password did not match<a style="float:right;"
								href="?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&pill=manage-users&zone=all-users#tabs-utils">clear</a>
						</div>
					<? elseif (isset($_GET['error']) && $_GET['error'] == 3): ?>
						<div class="alert alert-danger">A user with that email address already exists<a style="float:right;"
								href="?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&pill=manage-users&zone=all-users#tabs-utils">clear</a>
						</div>
					<? elseif (isset($_GET['error'])): ?>
						<div class="alert alert-danger"><? echo $_GET['error']; ?><a style="float:right;"
								href="?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&pill=manage-users&zone=all-users#tabs-utils">clear</a>
						</div>
					<? endif; ?>
					<? if (isset($_REQUEST['action']) && 'deleted' == $_REQUEST['action']): ?>
						<div class="alert alert-success">User has been deleted<a style="float:right;"
								href="?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&pill=manage-users&zone=all-users#tabs-utils">clear</a>
						</div>
					<? endif ?>
					<? if (isset($_REQUEST['action']) && 'added' == $_REQUEST['action']): ?>
						<div class="alert alert-success">User has been added and is present in the list below<a
								style="float:right;"
								href="?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&pill=manage-users&zone=all-users#tabs-utils">clear</a>
						</div>
					<? endif ?>
					...
					<div class="admin-nav-wrap">
						...
						<? if ($start !== 0 || (count($users) - 1) == $total): ?>
							<small style="float:left;padding-top:4px;">
								<? if ($start !== 0): ?>
									<span class="prev"><a
											href="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&zone=all-users&pill=manage-users&start=<?= $start - $total ?>&total=<?= $total ?>#tabs-utils">Prev
											page</a></span> &nbsp;
								<? endif ?>
								<b class="total"><?= $start + 1 ?> - <?= $start + $count ?></b>
								<? if (count($users) - 1 == $total): ?>
									&nbsp; <span class="prev"><a
											href="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&zone=all-users&pill=manage-users&start=<?= $start + $total ?>&total=<?= $total ?>#tabs-utils">Next
											page</a></span>
								<? endif ?>
								<form style="display:inline-block" class="jump-form">
									<span>&nbsp;&nbsp;&nbsp;&nbsp;Go to page: &nbsp; </span>
									<input style="text-align:right" placeholder="<?= $start / $total + 1 ?>" type="text"
										class="jump-to-page" size="2" />
								</form>
								&nbsp; &nbsp; &nbsp; &nbsp;
							</small>
						<? endif ?>
						<form class="form-inline user-search-form"
							style="<?= (($start !== 0 || (count($users) - 1) == $total) ? 'float:right;' : 'float:left;') ?>">
							<div class="form-group form-group-sm">
								...
								<small style="padding-left:3px;"><a
										href="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&zone=all-users&pill=manage-users#tabs-utils">clear</a></small>
							</div>
						</form>
						<br clear="both" />
					</div>
					...
					<form class="form-inline" action="<?= confirm_slash(base_url()) ?>system/dashboard" method="post"
						onsubmit="checkAddUserForm(this);">
						...
						<div id="manage-users-add-new" class="form-group form-group-sm">
							<input tabindex="1" class="form-control" type="text" name="email" value=""
								placeholder="Email address" />&nbsp;
							<input tabindex="2" class="form-control" type="text" name="fullname" value=""
								placeholder="Full name" />&nbsp;
							<input tabindex="3" class="form-control" type="password" name="password_1" value=""
								placeholder="Password" />&nbsp;
							<input tabindex="4" class="form-control" type="password" name="password_2" value=""
			...
			<div class="section" id="google-authenticator">
				<?php if ('google-authenticator' == $pill): ?>
					<form
						action="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : 0) ?>&zone=utils&pill=google-authenticator#tabs-utils"
						method="post">
						...
						<div>
							...
							?></div>
							<div style="width:50%;float:right;border-left:solid 1px #aaaaaa;padding-left:20px;">
			...
			<div class="section" id="disallowed-emails">
				<?php if ('disallowed-emails' == $pill): ?>
					<form
						action="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : 0) ?>&zone=utils&pill=disallowed-emails#tabs-utils"
						method="post" id="disallowed_emails_form">
						<input type="hidden" name="zone" value="utils" />
			...
			<div class="section" id="manage-books">
				<?php if ('manage-books' == $pill): ?>
					...
					<style>
						...
						#manage-books-add-new input,
						#manage-books-add-new select {
							margin-bottom: 4px;
						}
					...
					<? if (isset($_GET['error']) && $_GET['error'] == 1): ?>
						<div class="alert alert-warning">Title is a required field<a style="float:right;"
								href="?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&pill=manage-books&zone=all-books#tabs-utils">clear</a>
						</div>
					<? endif; ?>
					<? if (isset($_REQUEST['action']) && 'deleted' == $_REQUEST['action']): ?>
					...
					<div class="admin-nav-wrap">
						...
						<? if ($start !== 0 || (count($books) - 1) == $total): ?>
							<small style="float:left;padding-top:4px;">
								<? if ($start !== 0): ?>
									<span class="prev"><a
											href="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&pill=manage-books&zone=all-books&amp;start=<?= $start - $total ?>&amp;total=<?= $total ?>#tabs-utils">Prev
											page</a></span> &nbsp;
								<? endif ?>
								<b class="total"><?= $start + 1 ?> - <?= $start + $count ?></b>
								<? if (count($books) - 1 == $total): ?>
									&nbsp; <span class="next"><a
											href="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&pill=manage-books&zone=all-books&amp;start=<?= $start + $total ?>&amp;total=<?= $total ?>#tabs-utils">Next
											page</a></span>
								<? endif ?>
								<form style="display:inline-block" class="jump-form">
									<span>&nbsp;&nbsp;&nbsp;&nbsp;Go to page: &nbsp; </span>
						...
						<? endif ?>
						<form class="form-inline book-search-form"
							style="<?= (($start !== 0 || (count($users) - 1) == $total) ? 'float:right;' : 'float:left;') ?>">
							<div class="form-group form-group-sm">
								...
								<small style="padding-left:3px;"><a
										href="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book->book_id)) ? $book->book_id : 0) ?>&pill=manage-books&zone=all-books&amp;start=<?= $start ?>&amp;total=<?= $total ?>#tabs-utils">clear</a></small>
							</div>
						</form>
						<br clear="both" />
					</div>
			...
			<div class="section" id="generate-email-list">
				<?php if ('generate-email-list' == $pill): ?>
					<form
						action="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : 0) ?>&zone=utils&pill=generate-email-list#tabs-utils"
						method="post">
						<input type="hidden" name="zone" value="utils" />
			...
			<div class="section" id="recreate-book-folders">
				<?php if ('recreate-book-folders' == $pill): ?>
					<form
						action="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : 0) ?>&zone=utils&pill=recreate-book-folders#tabs-utils"
						method="post">
						<input type="hidden" name="zone" value="utils" />
			...
			<div class="section" id="list-all-users">
				<?php if ('list-all-users' == $pill): ?>
					<form
						action="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : 0) ?>&zone=utils&pill=list-all-users#tabs-utils"
						method="post">
						<input type="hidden" name="zone" value="utils" />
						<input type="hidden" name="action" value="get_recent_users" />
						<h4>List all users</h4>
						You can delete users and their associated books from this list; links will open in the All Users tab
						in new window.<br /><br />
						<div class="div_list"><?php
						...
						?>
							...
							<input type="button" value="Generate" class="btn btn-primary"
								onclick="this.disabled=true;$(this).parent().find('form:first').trigger('submit');" />
						<?php endif; ?>
					</div>
					...
					<div class="section" id="list-recent-pages">
						<?php if ('list-recent-pages' == $pill): ?>
							<form
								action="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : 0) ?>&zone=utils&pill=list-recent-pages#tabs-utils"
								method="post">
								<input type="hidden" name="zone" value="utils" />
							...
							<div class="section" id="list-all-books">
								<?php if ('list-all-books' == $pill): ?>
									<form
										action="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : 0) ?>&zone=utils&pill=list-all-books#tabs-utils"
										method="post">
										<input type="hidden" name="zone" value="utils" />
										<input type="hidden" name="action" value="get_recent_book_list" />
										<h4>List all books</h4>
										Delete books and their creators from this list; links will open in the All Books or
										All Users tab in new window.<br /><br />
										<div class="div_list"><?php
									...
									<div class="section" id="normalize-id2val">
										<?php if ('normalize-id2val' == $pill): ?>
											<form
												action="<?= confirm_slash(base_url()) ?>system/dashboard?book_id=<?= ((isset($book) && !empty($book)) ? $book->book_id : 0) ?>&zone=utils&pill=normalize-id2val#tabs-utils"
												method="post">
												<input type="hidden" name="zone" value="utils" />
												<input type="hidden" name="action" value="normalize_predicate_table" />
												<h4>Normalize id2val table</h4>
												Will remove duplicates from the ARC2 id2val table so that predicate searches
												work as expected.<br /><br />
												<div class="div_list"><?php
												...
												?>
													<br />
													<input type="button" value="Normalize" class="btn btn-primary"
														onclick="this.disabled=true;$(this).parent().find('form:first').trigger('submit');" />
												<?php endif; ?>
											</div>
		</section>
